    ...
    def on_frame(self, controller):
        ...
        if i == 2 or i == 1:
            ...
            if confidence < 0.5:
                ...
                # Activate warning and set message for display
                signalWarning = True
                warningMessage = "L'acquisition est interrompue, l’efficacité de la Leap Motion est trop faible : "+str(int(confidence*10))+"%"
            ...
            if distance > 250:
                ...
                # Activate warning and set message for display
                signalWarning = True
                warningMessage = "L'acquisition est interrompue, la distance entre la Leap Motion et la main est trop élevée : "+str(int(distance))+"mm"
            ...
            for Joint_select in Finger_select[1]:                                   # Each joint chosen for each finger
                for Data_select in [Min, Value, Max]:                               # Value, Min, Max
                    ...
                    if Hands_Angle[Hand_select][Finger_select[0]][Joint_select][Data_select] != Data_select_initial:
                        ...
                        if Data_select == Min:
                            Results[Hand_select][Finger_select[0]][Joint_select][Data_select] = str(temp).zfill(3)
                        if Data_select == Max:
                            Results[Hand_select][Finger_select[0]][Joint_select][Data_select] = str(temp).zfill(3)
                        if Data_select == Value:
                            Results[Hand_select][Finger_select[0]][Joint_select][Data_select] = str(temp).zfill(3)
...
class upateThread(QtCore.QThread):
    ...
    def __init__(self,parent=None):
        ...
        global Acquisition                          # Define global marker of acquisition which define emission or not

    # Function for running thread
    def run(self):
        while True:
            self.progress.emit(Results)             # Emit angles calculated to GUI
            formatData(Hands_Angle)                 # Put values in correct format (3 digits)
...
class page(QtGui.QTabWidget, pageUi.Ui_TabWidget):
    ...
    def changeModeMainTableau(self):
        ...
        if self.modeMain:
            ...
            # Create icon
            newIcon = QtGui.QIcon()
            newIcon.addPixmap(QtGui.QPixmap(path_script+"filesUi/imagesUi/table.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        if not self.modeMain:
            ...
            self.message_D_3.setText(QtGui.QApplication.translate("TabWidget", "<html><head/><body><p align=\"center\"><span style=\" font-size:16pt;\">TABLEAU</span></p></body></html>", None, QtGui.QApplication.UnicodeUTF8))
            ...
            # Create icon
            newIcon = QtGui.QIcon()
            newIcon.addPixmap(QtGui.QPixmap(path_script+"filesUi/imagesUi/main.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def updateGraphiqueTableau(self):
        ...
        self.choix_dossier.setCurrentIndex(self.choix_dossier_2.currentIndex()+1)
        ...
        self.choix_dossier_3.setItemText(0,QtGui.QApplication.translate("TabWidget","Tous", None, QtGui.QApplication.UnicodeUTF8))
        k = 1
        ...
        if self.infos != []:
            ...
            for j in range(2, 58):
                if self.index_labels_F[j-2]!=int:
                    ...
                    self.choix_dossier_3.setItemText(k, QtGui.QApplication.translate("TabWidget",self.labels_F[self.index_labels_F[j-2]], None, QtGui.QApplication.UnicodeUTF8))    # in UTF8 because accents
                    k += 1

            # Delete all others data
            while self.choix_dossier_3.count()>number_angle+1:
                self.choix_dossier_3.removeItem(self.choix_dossier_3.count()-1)
            ...
            scene.addLine(-L/2+l1*10, H/2-l1*10, L/2-l1*10, H/2-l1*10, QtGui.QPen(QtCore.Qt.black, 2))
            scene.addLine(-L/2+l1*10, H/2-l1*10, -L/2+l1*10, H/2-l1*10-h*180, QtGui.QPen(QtCore.Qt.black, 2))
            ...
            for i in range(0,181):
                ...
                if i%5==0:
                    scene.addLine(-L/2+l1*10, H/2-l1*10-h*i,-L/2+l1*10-l*3, H/2-l1*10-h*i, QtGui.QPen(QtCore.Qt.black))
                    if i%10==0:

                        t = QtGui.QGraphicsTextItem()
                        t.setPos(-L/2+l1*10-l*8, H/2-l1*10-h*i-h*3)
                    ...
                    scene.addLine(-L/2+l1*10, H/2-l1*10-h*i,-L/2+l1*10-l, H/2-l1*10-h*i, QtGui.QPen(QtCore.Qt.black))
            ...
            # Draw graphic for following patient
                ...
                for i in range(1,dates_number+1):
                    ...
                    scene.addLine(-L/2+l1*10+l*i, H/2-l1*10,-L/2+l1*10+l*i, H/2-l1*10+h*3, QtGui.QPen(QtCore.Qt.black))
                    t = QtGui.QGraphicsTextItem()
                    t.setPos(-L/2+l1*10+l*i-20, H/2-h)
                    t.setPlainText(x[self.choix_dossier_3.currentIndex()-1][i-1])
                    font = QtGui.QFont()
                    font.setPointSize(8)
                    font.setFamily("Book Antiqua")
                    t.setFont(font)
                ...
                for i in range (0,len(y[self.choix_dossier_3.currentIndex()-1])):
                    index = int(60+y[self.choix_dossier_3.currentIndex()-1][i])
                    scene.addEllipse(abscissa[i], ordinates[index], 5, 5, QtGui.QPen(QtCore.Qt.darkBlue), QtGui.QBrush(QtCore.Qt.black))
                    if i != 0:
                        index_p = int(60+y[self.choix_dossier_3.currentIndex()-1][i-1])
                        scene.addLine(abscissa[i], ordinates[index], abscissa[i-1], ordinates[index_p], QtGui.QPen(QtCore.Qt.darkBlue))
    ...
    def changeModeGraphiqueTableau(self, patient):
        ...
        self.tableWidget_2.horizontalHeader().setDefaultSectionSize(size_H/(8+nb_date*2))
        ...
        for i in range (0, 14):
            ...
            item.setTextAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)
            item.setBackground(QtGui.QColor(245, 245, 255))
            self.tableWidget_2.setItem(2*i+1, (8+nb_date*2)/2+2, item)
            item = QtGui.QTableWidgetItem("")
            item.setTextAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)
            item.setBackground(QtGui.QColor(245, 245, 255))
            self.tableWidget_2.setItem(2*i+1, (8+nb_date*2)/2+3, item)
            ...
            item.setTextAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)
            item.setBackground(QtGui.QColor(245, 245, 255))
            self.tableWidget_2.setItem(2*i+2, (8+nb_date*2)/2+2, item)
            item = QtGui.QTableWidgetItem("")
            item.setTextAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)
            item.setBackground(QtGui.QColor(245, 245, 255))
            self.tableWidget_2.setItem(2*i+2, (8+nb_date*2)/2+3, item)
        ...
        for i in range(8, len(self.infos[patient])):
            ...
            item.setTextAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)
            item.setBackground(QtGui.QColor(245, 245, 255))
            self.tableWidget_2.setItem(0, (8+nb_date*2)/2+3+i-8, item)
            for j in range(2, 16):
            ...
            for j in range(16, 30):
                ...
                item.setTextAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)
                item.setBackground(QtGui.QColor(245, 245, 255))
                self.tableWidget_2.setItem(2*(j-16)+2, ((8+nb_date*2)/2)+3+i-8, item)
            ...
            for j in range(44, 58):
                ...
                item.setTextAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)
                item.setBackground(QtGui.QColor(245, 245, 255))
                self.tableWidget_2.setItem(2*(j-44)+1, ((8+nb_date*2)/2)+3+i-8, item)
    ...
    def saveDataSuivi(self):
        ...
        if fileName:
            ...
            # Do the reading
            file1 = open(path_script+'Results'+'/'+self.choix_dossier_2.currentText()+'.csv', 'rb')
    ...
    def updateBegin(self):
        ...
        global warningMessage
        mainPage.setMinimumSize(QtCore.QSize(mainPage.width(), mainPage.height()))
        self.centralwidget.setMinimumSize(QtCore.QSize(self.width(), self.height()))
        self.centralwidget.setMaximumSize(QtCore.QSize(self.width(), self.height()))
        self.setMinimumSize(QtCore.QSize(self.width(), self.height()))
        self.label_3_.setMinimumSize(QtCore.QSize(self.width()*0.8, self.height()*0.95))
        self.label_3_.setMaximumSize(QtCore.QSize(self.width()*0.8, self.height()*0.95))
    ...
    def readLabels(self):
        ...
        if os.path.exists(path_script+'Results'):                       # Read data in all files in folder of results
            fileNames = glob.glob(path_script+'Results' + '/*.csv')     # Extract all files of folder
            for i in range (0,len(fileNames)):                          # Travel each file of folder
                fileName =  fileNames[i].replace('\\','/')              # Correct file name
                if os.path.exists(fileName):                            # Check if file exist
                    bottle_list = []                                    # Initialisation variable of each file
                    with open(fileName, 'rb') as b:                     # Open csv file of folder
                        bottles = csv.reader(b)                         # Read all lines of file
                        bottle_list.extend(bottles)                     # Save all lines of file
                        b.close()                                       # Close csv file of folder
                    bottle.append(bottle_list)                          # Add data read at the end of list

        for i in range (0,len(bottle)):                                 # Travel each file data
            info_list = [bottle[i][0][0][7:],                           # Save first name
                         bottle[i][1][0][4:],                           # Save last name
                         bottle[i][2][0][18:],                          # Save birth date
                         bottle[i][3][0][5:],                           # Save sex
                         bottle[i][4][0][18:],                          # Save report number
                         bottle[i][5][0][20:],                          # Save update date
                         bottle[i][6][0][21:],                          # Save update hour
                         bottle[i][7][0][15:]]                          # Save user

            for j in range (13,len(bottle[i])):                         # For older sessions
                info_list.append(bottle[i][j][0].split(';'))           # Save old angles measured

            self.infos.append(info_list)                                # Add angles with other data

            if self.choix_dossier.count() <= len(bottle):               # Check if there are all patient item
                self.choix_dossier.addItem("")                          # Add new patient item to combo box
                self.choix_dossier_2.addItem("")                        # Add new patient item to combo box
    ...
    def disableData(self):
        ...
        if self.choix_dossier.currentText() != "Nouveau patient":
            ...
            self.choix_dossier_2.setCurrentIndex(self.choix_dossier.currentIndex()-1)
            ...
            self.date_naissance.setDate(QtCore.QDate(int(self.infos[patient][2].split('-')[0]),int(self.infos[patient][2].split('-')[1]), int(self.infos[patient][2].split('-')[2])))
            if self.infos[patient][3]=="M":
                self.sexe.setCurrentIndex(0)
            if self.infos[patient][3]=="F":
                self.sexe.setCurrentIndex(1)
            self.derniere_seance.setDate(QtCore.QDate(int(self.infos[patient][len(self.infos[patient])-1][0].split('-')[0]),int(self.infos[patient][len(self.infos[patient])-1][0].split('-')[1]), int(self.infos[patient][len(self.infos[patient])-1][0].split('-')[2])))
    ...
    def startAcquisition(self):
        ...
        if Acquisition :
            ...
            if not signalWarning :
                ...
                mainPage.statusbar.showMessage(QtGui.QApplication.translate("TabWidget","", None, QtGui.QApplication.UnicodeUTF8))

                # Create icon
                newIcon = QtGui.QIcon()
                newIcon.addPixmap(QtGui.QPixmap(path_script+"filesUi/imagesUi/green_circle.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
            ...
            newIcon = QtGui.QIcon()
            newIcon.addPixmap(QtGui.QPixmap(path_script+"filesUi/imagesUi/stop.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        if not Acquisition :
            ...
            mainPage.statusbar.showMessage(QtGui.QApplication.translate("TabWidget",warningMessage, None, QtGui.QApplication.UnicodeUTF8))

            # Create icon
            newIcon = QtGui.QIcon()
            newIcon.addPixmap(QtGui.QPixmap(path_script+"filesUi/imagesUi/red_circle.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
            ...
            newIcon = QtGui.QIcon()
            newIcon.addPixmap(QtGui.QPixmap(path_script+"filesUi/imagesUi/start.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
                ...
                if reply == QtGui.QMessageBox.Yes :
                    ...
                    # If report number is not empty, save data
                        ...
                        if set(sum(sum(sum(Results_to_save, []), []), [])) == {' '}:
                            ...
                            self.save()
                            self.readLabels()
                            self.choix_dossier.setCurrentIndex(self.choix_dossier.findText(unicode(self.numero_dossier_3.text()), QtCore.Qt.MatchFixedString))
    ...
    def saveData(self):
        ...
        if fileName:
            ...
            with open(fileName , 'wb') as fp:
                ...
                a.writerow(["Date de la seance", str(datetime.datetime.fromtimestamp(time.time())).split(' ')[0]])
                a.writerow(["Heure", str(datetime.datetime.fromtimestamp(time.time())).split(' ')[1][:8]])
                a.writerow(["Ergotherapeute", QtGui.QApplication.translate("TabWidget", User, None, QtGui.QApplication.UnicodeUTF8)])
                a.writerow("---------------------------------------------------------")
                a.writerow([" ",
                            "Flexion", " ",
                            " ", " ", " ",
    ...
    def save(self):
        ...
        data = [str(datetime.datetime.fromtimestamp(time.time())).split(' ')[0],
                str(datetime.datetime.fromtimestamp(time.time())).split(' ')[1][:8],
                Results_to_save[Left][Thumb][MP][Max],Results_to_save[Left][Thumb][IPP][Max],
                Results_to_save[Left][Index][MP][Max],Results_to_save[Left][Index][IPP][Max],Results_to_save[Left][Index][IPD][Max],
                Results_to_save[Left][Middle][MP][Max],Results_to_save[Left][Middle][IPP][Max],Results_to_save[Left][Middle][IPD][Max],
                Results_to_save[Left][Ring][MP][Max],Results_to_save[Left][Ring][IPP][Max],Results_to_save[Left][Ring][IPD][Max],
        ...
        if os.path.exists(path_script+'Results'+'/'+ Report_Number +'.csv'):
            replace_row(path_script+'Results'+'/'+ Report_Number +'.csv',5, ["Date de mise a jour", str(datetime.datetime.fromtimestamp(time.time())).split(' ')[0]])
            replace_row(path_script+'Results'+'/'+ Report_Number +'.csv',6, ["Heure de mise a jour", str(datetime.datetime.fromtimestamp(time.time())).split(' ')[1][:8]])
            replace_row(path_script+'Results'+'/'+ Report_Number +'.csv',7, ["Ergotherapeute", User])
            ...
            while line >=13:
                ...
                if bottle_list[line][0][:10]==str(datetime.datetime.fromtimestamp(time.time())).split(' ')[0]:
                    ...
                    replace_row(path_script+'Results'+'/'+ Report_Number +'.csv',line, data)
        ...
        if not os.path.exists(path_script+'Results'+'/'+ Report_Number +'.csv'):
            with open(path_script+'Results'+'/'+ Report_Number +'.csv' , 'wb') as fp:
                ...
                a.writerow(["Sexe", Sex])
                a.writerow(["Numero de dossier", Report_Number])
                a.writerow(["Date de mise a jour", str(datetime.datetime.fromtimestamp(time.time())).split(' ')[0]])
                a.writerow(["Heure de mise a jour", str(datetime.datetime.fromtimestamp(time.time())).split(' ')[1][:8]])
    ...
    def updateText(self,text):
        ...
        self.updateThread.progress.connect(self.zoomDelete,QtCore.Qt.BlockingQueuedConnection)
        self.i += 1
        # For each angle
        ...
        if Acquisition :
            ...
            if signalWarning :
                ...
                # Create icon
                newIcon = QtGui.QIcon()
                newIcon.addPixmap(QtGui.QPixmap(path_script+"filesUi/imagesUi/mauve_circle.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
            ...
            for Finger_select in User_Choices[1]:
                for Joint_select in Finger_select[1]:
                    for Data_select in [Min, Value, Max]:
                        ...
                        button = getattr(self, self.buttons[Hand_select][Finger_select[0]][Joint_select][Data_select])

                        # Check if user select this angle
                        if button.isChecked():

                            # Not display minimum if user select flexion movement
                            if self.flexion_2.isChecked() and Data_select == Min:
                                label.setVisible(False)
                                text[Hand_select][Finger_select[0]][Joint_select][Data_select]=""

                            # Display and save maximum if user select flexion movement
                            if self.flexion_2.isChecked() and Data_select == Max:
                                label.setVisible(True)
                                Results_to_save[Hand_select][Finger_select[0]][Joint_select][Data_select]=text[Hand_select][Finger_select[0]][Joint_select][Data_select]

                            # Not display maximum if user select extension movement
                            if self.extension_2.isChecked() and Data_select == Max:
                                label.setVisible(False)
                                text[Hand_select][Finger_select[0]][Joint_select][Data_select]=""

                            # Display and save minimum if user select extension movement
                            if self.extension_2.isChecked() and Data_select == Min:
                                label.setVisible(True)
                                Results_to_save[Hand_select][Finger_select[0]][Joint_select][Data_select]=text[Hand_select][Finger_select[0]][Joint_select][Data_select]

                            # Display and save minimum if user select flexion movement
                            ...
                            # For others angles
                                ...
                                if Data_select == Min or Data_select == Max:
                                    item = QtGui.QTableWidgetItem(text[Hand_select][Finger_select[0]][Joint_select][Data_select])
                                    item.setTextAlignment(QtCore.Qt.AlignHCenter | QtCore.Qt.AlignVCenter)
                                    item.setBackground(QtGui.QColor(245, 245, 255))
                                    self.tableWidget.setItem(self.cellV[Hand_select][Finger_select[0]][Joint_select][Data_select], self.cellH[Hand_select][Finger_select[0]][Joint_select][Data_select], item)

                            # Display angle
                            label.setText (text[Hand_select][Finger_select[0]][Joint_select][Data_select])
    ...
    def zoomDelete(self, text):
        ...
        if (self.handDelete == Left or self.handDelete == Right ):
            ...
            if self.dataDelete == Min:
                Hands_Angle[self.handDelete][self.fingerDelete][self.jointDelete][self.dataDelete]=180
            if self.dataDelete == Max:
                Hands_Angle[self.handDelete][self.fingerDelete][self.jointDelete][self.dataDelete]=-180
            if self.dataDelete == Value:
                Hands_Angle[self.handDelete][self.fingerDelete][self.jointDelete][self.dataDelete]=0
            Results[self.handDelete][self.fingerDelete][self.jointDelete][self.dataDelete]="***"

        # Check if user click an left angle for delete
        if self.handZoom == Left:
            # Reset value of left zoom
            self.zoom_gauche.setText(text[self.handZoom][self.fingerZoom][self.jointZoom][self.dataZoom])
            # Clear right zoom
            self.zoom_droite.setText("")
        ...
        if self.handZoom == Right:
            # Reset value of left zoom
            self.zoom_droite.setText(text[self.handZoom][self.fingerZoom][self.jointZoom][self.dataZoom])
            # Clear left zoom
            self.zoom_gauche.setText("")
